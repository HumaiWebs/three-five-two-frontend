...
const DeleteProductButton = ({ productId ,productName}: Props) => {
  const [open, setOpen] = useState(false);

  const { mutate, status } = useMutation({
    mutationFn: async () => {
      return (await http.delete("/product/" + productId)).data;
    },
    onSuccess: (data) => {
      if (data.success) {
        setOpen(false);
  ...
  return (
    ...
    >
      <div>
        <p>Are you sure you want to <strong className="text-red-700">delete</strong> <strong>{productName}</strong>?</p>
